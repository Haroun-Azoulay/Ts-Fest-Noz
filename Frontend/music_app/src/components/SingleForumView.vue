...
const postSingle = ref<Post | null>(null);
const authToken = ref<string | null>(localStorage.getItem("authToken"));
const commentarySingle = ref<Commentary[]>([]);
...
onMounted(async () => {
  try {
    ...
    const { payload } = useJwt<JwtPayloadCustom>(authToken.value || '');
    if (payload.value?.userId) {
      userId = payload.value.userId;
    } else {
      console.error("Impossible d'extraire l'ID utilisateur du JWT.");
      return;
    }
    ...
    const postId = route.params.idpost as string;
    if (postId) {
