<template>
  <div>
    ...
    <h2 class="" style="text-align: center; background: #371990; margin-bottom: 0">
      Entrez vos informations et situez<br />
      <span class="id-color">votre événement sur la carte !</span>
    </h2>
    ...
    <section class="flex flex-col lg:flex-row justify-center items-center">
      <div id="add-event-map" class="row" style="height: 0vh">
        ...
        >
          ...
          <p class="text-m text-white mb-4 text-center">
            Nom de l'emplacement : {{ result.place }} {{ result.postalCode }} {{ result.city }}
            {{ result.country }}
          </p>
          ...
          <input
            type="datetime-local"
            class="mb-4 w-full h-12 p-2 rounded"
            name="meeting-time"
            v-model="meetingTime"
            min="2024-07-07T00:00"
            max="2025-07-14T00:00"
          />
          ...
          <textarea
            class="mb-1 w-full h-24 p-2 rounded"
            v-model="event_txt"
            type="text"
...
const styleColor = computed(() => {
  switch (selectedStyle.value) {
    case 'jazz': return '#00FF7F'
    case 'rap-rnb': return '#6A5ACD'
    case 'classique': return '#FFC0CB'
    case 'reggae': return '#D2691E'
    case 'pop-rock': return '#FFE4E1'
    case 'country': return '#B0C4DE'
...
const addPoint = async () => {
  try {
    if (!result.value) throw new Error('Aucune donnée de géocodage disponible.')
    ...
    Point.url_point = `http://localhost:5173/event/${eventId}`

    await ApiService.post('/city/add-point', Point, {
      headers: { Authorization: `Bearer ${authToken}` }
    })
...
const updateMap = (coordinates: [number, number]) => {
  if (map) map.remove()
